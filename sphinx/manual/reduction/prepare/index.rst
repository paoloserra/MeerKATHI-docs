...
* if conversion from HDF5/MVF format to .MS is necessary it can be enabled through the
  *get_data:mvftoms* parameter; further options allow users to create a .MS.TAR file, to
  convert only visibilities in a channel range, or to discard cross-polarisation products;
...
in the :ref:`observation_config` worker. In fact, MeerKATHI can automatically extract most
...
*observation_config:reference_antenna* parameter.
